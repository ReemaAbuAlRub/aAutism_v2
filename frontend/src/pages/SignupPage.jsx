...
//               />
//               <button
//                 className="modern-btn signin-btn"
...
export default function SignupPage({ setUsername, darkMode }) {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
  ...
  const handleBirthDate = (e) => {
    ...
    const m = todayD.getMonth() - birth.getMonth();
    if (m < 0 || (m === 0 && todayD.getDate() < birth.getDate())) age--;
    setFormData((prev) => ({ ...prev, birthDate, age }));
  };

  const handleSignup = async () => {
    if (!formData.email || !formData.password || !formData.birthDate || !formData.firstName || !formData.lastName) {
      alert(t("alerts.fillAllFields"));
      return;
    }
    setLoading(true);
    try {
      const payload = {
        ...
        autism_level: formData.autismLevel || null,
      };
  ...
  return (
    <div className={`signup-page ${darkMode ? "dark" : ""}`}>
      <div className="page-layout">
        <LeftPanel title={t("signupPage.signupTitle")} />
        <div className="right-section">
          <div className="form-box modern-form">
            <h2 className="form-title">{t("signupPage.signupTitle")}</h2>
            ...
            <input type="date" name="birthDate" max={today} onChange={handleBirthDate} />
            {formData.age && (
              <p className="age-display">{t("signupPage.ageDisplay", { age: formData.age })}</p>
            )}
            ...
            <select name="autismLevel" onChange={handleChange} defaultValue="">
              ...
              <option value="LVL1">{t("signupPage.signupAutismLevel1")}</option>
              <option value="LVL2">{t("signupPage.signupAutismLevel2")}</option>
              <option value="LVL3">{t("signupPage.signupAutismLevel3")}</option>
            </select>
            ...
            <button className="modern-btn signup-btn" onClick={handleSignup} disabled={loading}>
              {loading ? t("loading") : t("signupPage.signupBtn")}
            </button>
          </div>
        </div>
